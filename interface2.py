...
class GraphInterface:
    def __init__(self, root):
        ...
        tk.Button(control_frame, text="Seleccionar nodo inicial", command=self.select_start_node).pack(pady=5)
        tk.Button(control_frame, text="Seleccionar nodo final", command=self.select_end_node).pack(pady=5)
        tk.Button(control_frame, text="Camino más corto", command=self.shortest_path).pack(pady=5)
    ...
    def on_click(self, event):
        ...
        if self.mode == "add_node":
            ...
            if self.selected_node is None:
                ...
                segment_name = simpledialog.askstring("Nombre del segmento", "Introduce el nombre del segmento:")
                if segment_name:
                    success = AddSegment(self.graph, segment_name, self.selected_node.name, clicked_node.name)
                    if success:
                        self.draw()
                    else:
    ...
    def save_to_file(self):
        ...
        if path:
            SaveGraphToFile(self.graph, path)
    ...
    def select_start_node(self):
        name = simpledialog.askstring("Nodo inicial", "Introduce el nombre del nodo de inicio:")
        node = self.get_node_by_name(name)
        if node:
            self.start_node = node
            messagebox.showinfo("Nodo inicial", f"Nodo inicial seleccionado: {node.name}")
        else:
            messagebox.showwarning("Error", "Nodo no encontrado.")

    def select_end_node(self):
        name = simpledialog.askstring("Nodo final", "Introduce el nombre del nodo de destino:")
        node = self.get_node_by_name(name)
        if node:
            self.end_node = node
            messagebox.showinfo("Nodo final", f"Nodo final seleccionado: {node.name}")
        else:
            messagebox.showwarning("Error", "Nodo no encontrado.")
    ...
    def shortest_path(self):
        if not self.start_node or not self.end_node:
            messagebox.showwarning("Faltan nodos", "Debes seleccionar un nodo inicial y uno final.")
            return

        path = FindShortestPath(self.graph, self.start_node.name, self.end_node.name)
        if path:
            self.ax.clear()
            PlotPath(self.graph, path, self.ax)
            self.canvas.draw()
            messagebox.showinfo("Camino encontrado", f"Camino encontrado: {' -> '.join([n.name for n in path.nodes])}")
        else:
            messagebox.showwarning("Sin camino", "No existe un camino entre los nodos seleccionados.")
        self.draw()
        PlotPath(self.graph, path, self.ax)
        self.canvas.draw()
